...
priv_memfd_backend_memory_alloc(HostMemoryBackend *backend, Error **errp)
{
    ...
    g_warning("backend_memory_alloc: marker 0, size: 0x%lx", backend->size);
    ...
    g_free(name);
    g_warning("backend_memory_alloc: marker 1, name: %s, flags: 0x%x, fd: %d, priv_fd: %d",
              name, ram_flags, fd, priv_fd);
