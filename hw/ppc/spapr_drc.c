                                    ...
                                    sPAPRDRIsolationState state)
{
    ...
    error_report("drc: %d, isolation state marker 0", get_index(drc));
     ...
     */
    error_report("drc: %d, isolation state marker 1", get_index(drc));
    ...
    error_report("drc: %d, isolation state marker 2", get_index(drc));
    if (drc->isolation_state == SPAPR_DR_ISOLATION_STATE_ISOLATED) {
        error_report("drc: %d, isolation state marker 3", get_index(drc));
        ...
        if (drc->awaiting_release) {
            if (drc->configured) {
                error_report("drc: %d, isolation state marker 4", get_index(drc));
                trace_spapr_drc_set_isolation_state_finalizing(get_index(drc));
                drck->detach(drc, DEVICE(drc->dev), drc->detach_cb,
                             drc->detach_cb_opaque, NULL);
            } else {
                error_report("drc: %d, isolation state marker 5", get_index(drc));
                                     ...
                                     sPAPRDRAllocationState state)
{
    ...
    error_report("drc: %d, allocation state marker 0", get_index(drc));
    ...
    error_report("drc: %d, allocation state marker 1", get_index(drc));
    if (drc->type != SPAPR_DR_CONNECTOR_TYPE_PCI) {
        drc->allocation_state = state;
        error_report("drc: %d, allocation state marker 2", get_index(drc));
        if (drc->awaiting_release &&
            drc->allocation_state == SPAPR_DR_ALLOCATION_STATE_UNUSABLE) {
            error_report("drc: %d, allocation state marker 3", get_index(drc));
            trace_spapr_drc_set_allocation_state_finalizing(get_index(drc));
            drck->detach(drc, DEVICE(drc->dev), drc->detach_cb,
                         drc->detach_cb_opaque, NULL);
        } else if (drc->allocation_state == SPAPR_DR_ALLOCATION_STATE_USABLE) {
            error_report("drc: %d, allocation state marker 4", get_index(drc));
...
static bool spapr_drc_needed(void *opaque)
{
    ...
    if (drc->migration_needed_cb &&
        !drc->migration_needed_cb(drc->migration_needed_opaque)) {
        return false;
    }
