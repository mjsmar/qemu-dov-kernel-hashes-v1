...
static gpointer region_key(uint32_t bdf, uint32_t bar)
{
    return GINT_TO_POINTER((bdf << 8) | bar);
}

hwaddr spapr_pci_resources_request_region(sPAPRPCIResources *res,
                                          sPAPRPCIResourceType rtype,
                                          ...
                                          uint32_t bdf, int bar)
{
    ...
    gpointer key = region_key(bdf, bar);

    g_assert_cmpint(rtype, <, SPAPR_PCI_RESOURCE_TYPE_MAX);
    g_assert_true(res->range[rtype].enabled);
